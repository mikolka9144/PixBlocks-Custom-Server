...
namespace Pix_API
{
	public class APIServerResolver
	{
		...
		public APIServerResolver(ICommandRepository logic, IUserDatabaseProvider userDatabaseProvider)
		{
			this.logic = logic;
		...
		private bool CallerhasPermissionToRun(MethodInfo method, object[] method_arguments)
		{
			if (method.GetParameters().Last().ParameterType == typeof(AuthorizeData))
			{
				...
				User user = userDatabaseProvider.GetUser(authorizeData.UserId);
				bool num = authorizeData.PasswordMD5 == user.Md5Password;
				bool flag = authorizeData.ExplicitPassword == user.Student_explicitPassword && user.Student_isStudent;
				return num || flag;
			}
			return true;
		}
